...
const supabaseClient = window.supabase ? window.supabase.createClient(SUPABASE_URL, SUPABASE_KEY) : null;
        ...
        window.addEventListener('DOMContentLoaded', () => {
            ...
            if (urlParams.get('p')) { 
                ...
                // Check auth before showing admin
                if (supabaseClient) {
        ...
        function scanAndRecover() {
            ...
            linkKeys.forEach(key => {
                ...
                if(data && Array.isArray(data)) {
                    data.forEach(item => {
                        ...
                        if(!secureLinks.find(l => String(l.id) === String(item.id))) { secureLinks.push(item); recoveredLinks++; }
                    });
                }
            });
            ...
            if(pData && Array.isArray(pData)) {
                pData.forEach(item => { if(!secureCustomLists.find(p => String(p.id) === String(item.id))) { secureCustomLists.push(item); recoveredPlaylists++; } });
            }
        ...
        function saveLinkOrder() {
            ...
            newOrderIds.forEach(id => {
                const link = secureLinks.find(l => String(l.id) === String(id));
            ...
            const doSave = () => {
                ...
                if (item.passwords.length !== newPasswords.length) {
                    ...
                    for (let i = 0; i < newPasswords.length; i++) {
                        ...
                        const oldP = item.passwords.find(p => String(p.id) === String(newP.id));
        ...
        function saveCustomListsOrder() {
            ...
            rows.forEach(tr => {
                ...
                const found = secureCustomLists.find(l => String(l.id) === String(id));
        ...
        function addSelectedToCustomList() {
            ...
            let list = secureCustomLists.find(p => String(p.id) === String(listId)); if(!list) return;
        ...
        function findPasswordAndLink(pwdId) { for(let l of secureLinks) { const p = l.passwords.find(pw => String(pw.id) === String(pwdId)); if(p) return { link: l, pwd: p }; } return null; }

        function openCustomListModal(listId) {
            currentCustomListId = listId; const list = secureCustomLists.find(p => String(p.id) === String(listId)); if(!list) return;
        ...
        function copyCustomListFormatted() {
            const list = secureCustomLists.find(p => String(p.id) === String(currentCustomListId)); if(!list) return;
        ...
        function toggleReorderCustomListPwds(forceCancel = false) {
            ...
            const list = secureCustomLists.find(p => String(p.id) === String(currentCustomListId));
            if (!list) return;
        ...
        function setupEditCustomListSourceAutocomplete() {
            ...
            input.addEventListener('input', function() {
                ...
                matches.forEach(m => {
                    ...
                    btn.onclick = () => {
                        if(m.type === 'group') {
                            const link = secureLinks.find(l => String(l.id) === String(m.linkId));
                            ...
                            const link = secureLinks.find(l => String(l.id) === String(m.linkId));
                            const p = link.passwords.find(pw => String(pw.id) === String(m.pwdId));
        ...
        function renderAddPwdPasswords() {
            ...
            const currentList = secureCustomLists.find(l => String(l.id) === String(currentCustomListId));
        ...
        function confirmAddPwdToList() {
            ...
            const list = secureCustomLists.find(l => String(l.id) === String(currentCustomListId));
        ...
        function importBackup(e) {
            ...
            reader.onload = function(evt) {
                try {
                    ...
                    linksToImport.forEach(impLink => { if(!secureLinks.find(l => String(l.id) === String(impLink.id))) { secureLinks.push(impLink); addedL++; } });
                    plsToImport.forEach(impPl => { if(!secureCustomLists.find(p => String(p.id) === String(impPl.id))) { secureCustomLists.push(impPl); addedP++; } });
                    localStorage.setItem('secure_links_v17', JSON.stringify(secureLinks)); saveListsLocallyAndSync();
...
async function configureAutoSaveFile() {
    ...
    try {
        ...
        await setHandle('autoSaveFile', fileHandle);
        customAlert("Arquivo de Backup configurado com sucesso!", "Sucesso");
    } catch (err) {
        console.error("Erro ao configurar Arquivo de Backup:", err);
    }
}

async function executeAutoSave() {
    try {
        const fileHandle = await getHandle('autoSaveFile');
        if (!fileHandle) {
            customAlert("Por favor, configure o arquivo de backup primeiro.", "Aviso");
            return;
        }
        ...
        customAlert("Edições salvas diretamente no arquivo " + fileHandle.name + "!", "Salvo com Sucesso");
    } catch (err) {
